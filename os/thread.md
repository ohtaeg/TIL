...
2. 컨텍스트 스위칭 지연 해소
  ...
  - 많은 Goroutine을 실행하기 위해 하나의 OS스레드에서 여러 Goroutine 이동한다.
  - 기존의 스레드/스레드 풀 방식보다 훨씬 가볍고 빠르지만 구현이 어렵다는 단점이 있다.

<br>

## 가상 스레드
[REF](https://www.javai.net/post/202209/java19-vt/)
- 자바는 위 문제점들을 개선하기 위해 Java 19부터 `프로젝트 룸`의 일부인 `가상 스레드`를 제공하여 고루틴과 코루틴의 격차를 줄이려고 노력하였다.
- Java19에서 기존에 사용했던 스레드를 `플랫폼 스레드`라고 하며 `플랫폼 스레드`는 여전히 시스템 커널 스레드와 일대일로 일치한다.
  - 즉, `플랫폼 스레드`는 OS 스케줄러에 의존하고, `가상 스레드`는 JDK 자체 스케줄러에 의존한다.
  - JDK의 스케줄러는 `가상 스레드`를 `플랫폼 스레드`에 할당한다.
- 아래 그림과 같이 다수(M)의 `가상 스레드`가 소수(N)의 `플랫폼 스레드`에서 실행된다.

  ![img.png](img/thread/img.png)

- 자바의 `가상 스레드`는 goroutine과 비슷하다.
  - `가상 스레드`를 사용할 때 JVM은 애플리케이션의 `가상 스레드`가 유휴 상태, 새 작업을 대기하는 상태일 경우 컴퓨팅 리소스를 할당할 수 있다.
- JDK의 `가상 스레드` 스케줄러는 ForkJoinPool(FIFO)을 모방하였다.
- `가상 스레드`는 항상 데몬 쓰레드이다.
  - 즉, JVM 프로세스를 완료될 때까지 활성 상태로 유지된다.
- 가상 스레드는 JVM에 의해 관리되므로 할당하는 데에 있어 시스템 콜이 필요하지 않고, Context Switching이 없다는 장점이 있다.
